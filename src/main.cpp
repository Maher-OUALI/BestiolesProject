...
int main()
{
   ...
   BestioleFactory factory=BestioleFactory();


   Bestiole* b1=factory.createBestiole(enum_Behavior::Spinner,enum_Sensor::Eyes);
   b1->setCoords(ecosysteme.getMilieu().getWidth()/2.0,ecosysteme.getMilieu().getHeight()/2.0);
   b1->setOrientationDeg(0.0);
   

   Bestiole* b2=factory.createBestiole(enum_Behavior::Braindead,enum_Sensor::Eyes);
